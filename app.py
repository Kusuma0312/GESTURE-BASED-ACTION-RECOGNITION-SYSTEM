...
from navi import detect_hand_gestures
from vol import hand_volume_control
from mov import hand_scroll_control
from pow import pprest
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if hand_sign_id==4:#brightnessontrol(v sign)
                   print(hand_sign_id)
                   cap.release()
                   cv.destroyAllWindows()
                   brightness_control()#brightnesscontrol
                if hand_sign_id==3:#ok pose
                    print(hand_sign_id)
                    cap.release()
                    cv.destroyAllWindows()
                    detect_hand_gestures()#navigation
                if hand_sign_id==2:#pointer pose
                    print(hand_sign_id)
                    cap.release()
                    cv.destroyAllWindows()
                    hand_volume_control()#volumecontrol
                if hand_sign_id==0:#open pose
                    print(hand_sign_id)
                    cap.release()
                    cv.destroyAllWindows()
                    hand_scroll_control()#readdocuments
                if hand_sign_id==1:# pose
                    print(hand_sign_id)
                    cap.release()
                    cv.destroyAllWindows()
                    pprest()#pptpresentationmode
